...
class AppleStrategy extends passport.Strategy {
                            ...
                            this.verifyNonce(req, jwtClaims.nonce_supported, jwtClaims.nonce, (err, ok) => {
                                ...
                                if (jwtClaims.email_verified) {
                                    profile.emailVerified =
                                        jwtClaims.email_verified === 'true' || jwtClaims.email_verified === true;
                                }

                                if (jwtClaims.is_private_email) {
                                    profile.isPrivateEmail =
                                        jwtClaims.is_private_email === 'true' || jwtClaims.is_private_email === true;
                                }
    ...
    parseErrorResponse(body) {
        ...
        if (json.error) {
            return new TokenError(json.error_description || json.error, json.error, json.error_uri);
        }
        return null;
    }
